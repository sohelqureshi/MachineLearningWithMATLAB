...
% % y = repmat([1:num_labels], m, 1) == repmat(y, 1, num_labels);
% % 
% % a1 = [ones(m,1) X]; % add a column of ones to X, 
% % z2 = a1*Theta1';
% % a2 = [ones(m,1) sigmoid(z2)];
% % a3 = sigmoid(a2*Theta2');
% % 
% % % Implementing matrix addition in this case is difficult, will start with
% % % simple sum for now and work back to vectorization as I figure it out
% % J = (1 / m) * sum(sum((-y .* log(a3) - (1-y) .* log(1-a3))));
% % % Setting the regularized version that incorporates lambdas
% % regTheta1 = Theta1(:,2:end);
% % regTheta2 = Theta2(:,2:end);
% % error = (lambda / (2*m)) * (sum(sum(regTheta1 .^2)) + sum(sum(regTheta2 .^2)));
% % J = J + error;
...
for j = 1:m,
    ...
    d3 = a3j - yj;
	d2 = Theta2'*d3' .* sigmoidGradient([1;Theta1 * a1j']);
	
    del1 = del1 + d2(2:end)*a1j;
	del2 = del2 + d3' * a2j;
end;
% 
